 ...
 * rotação/bearing, detecção de desvio de rota e formatação Waze.
 */
 ...
 */
export function minDistanceToPolyline(point, coordinates, thresholdMeters = 45) {
...
/**
 * Formata a distância de manobras de 100 em 100 metros (estilo Waze)
 * Ex: 870m -> 900 m, 420m -> 400 m, 120m -> 100 m, <50m -> 50 m
 */
export function formatWazeDistance(meters) {
  if (isNaN(meters) || meters === null || meters <= 0) return 'Agora';

  if (meters >= 1000) {
    const kmRounded = (Math.round(meters / 100) / 10).toFixed(1);
    return `${kmRounded} km`;
  }

  // De 100 em 100 metros
  if (meters >= 100) {
    const roundedHundreds = Math.round(meters / 100) * 100;
    return `${roundedHundreds} m`;
  }
...
/**
 * Formata distância geral para exibição em métricas (ex: 63.5 km)
 */
...
export function formatETA(secondsFromNow) {
  if (isNaN(secondsFromNow) || secondsFromNow === null) return '--:--';
